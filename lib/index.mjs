...
const parse = string => {
  ...
  if (!RegExpPrototypeTest(/[[\]]/, string)) {
    return StringPrototypeSplit(string, DELIMITER_CHAR)
  }
  const result = []
  if (string[0] === DELIMITER_CHAR) {
  ...
  [toStringSymbol]() {
    ...
    for (const key of ArrayPrototypeSafeValues(this[pathSymbol])) {
      ...
      result += expression !== undefined ? (
        !result && result[result.length - 1] === DELIMITER_CHAR ? `${DELIMITER_CHAR}${expression}` : expression
      ) : key ? (
        result && result[result.length - 1] !== DELIMITER_CHAR ? `${DELIMITER_CHAR}${key}` : key
      ) : DELIMITER_CHAR
    }
    return result
  }
