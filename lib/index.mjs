...
const parse = string => {
  ...
  if (!RegExpPrototypeTest(/[[\]]/, string)) {
    return StringPrototypeSplit(string, SEPARATOR_CHAR)
  }
  const result = []
  if (string[0] === SEPARATOR_CHAR) {
  ...
  [toStringSymbol]() {
    ...
    for (const key of ArrayPrototypeSafeValues(this[pathSymbol])) {
      ...
      result += expression !== undefined ? (
        !result && result[result.length - 1] === SEPARATOR_CHAR ? `${SEPARATOR_CHAR}${expression}` : expression
      ) : key ? (
        result && result[result.length - 1] !== SEPARATOR_CHAR ? `${SEPARATOR_CHAR}${key}` : key
      ) : SEPARATOR_CHAR
    }
    return result
  }
