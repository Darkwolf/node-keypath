...
class KeyPath {
  ...
  concat(...paths) {
    const path = toPathApply(paths)
    return new KeyPath(ArrayPrototypeConcat(this[pathSymbol], path))
  }

  push(...paths) {
    const path = this[pathSymbol]
    const keys = toPathApply(paths)
    for (const key of ArrayPrototypeSafeValues(keys)) {
      ArrayPrototypePush(path, key)
    }
    return path.length
  }
...
const isKeyPath = FunctionPrototypeSymbolHasInstance.bind(null, KeyPath)
...
const toPath = (...paths) => {
  ...
  for (const path of ArrayPrototypeSafeValues(paths)) {
    if (isKeyPath(path)) {
      for (const key of ArrayPrototypeSafeValues(path[pathSymbol])) {
